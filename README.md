...
``Obs.: O endereço do Serviço API que será acessado é: https://jsonplaceholder.typicode.com/users`` 
...
## Agregação
Agregação é um mecanismo que existe no OCELOT que ele pode fazer varias requisições pelo cliente e agregar o resultados dessas requisições em um novo obejto e retornar esse objeto completo.

Imagine a seguinte situação: Nós temos 3 serviços de API implementados: `Price, Book e Rating`. Para ter a informação completa nosso serviço web precisaria fazer 3 requisições, uma para cada serviços. Depois precisaria pegar o resultado de cada consulta e montar uma novo objeto para ter assim o Livro com preço e pontuação.
...
Observe acima que foi adicionado o grupo Aggregates e nele temos a configuraçõs abaixo:

* **RouteKeys**: Chave dos roteamentos que será chamado pelo rota de agregação
...
Na Classe `BookDetailsAggregator` descrita abaixo, observe que é necessário implentar a interface `IDefinedAggregator` do ocelot para que ele possa ser invocado quando a rota de agregação for acionado.
...
Observe que é implementado o método `Aggregate` reponsavel por toda a lógica de agregação assim que as rotas da agregação terminarem o seu processo. Em seguida é redirecionado para esse método onde devemos remontar um Json e responder a agregação desejada.
...
Observe na imagem acima que foi adicionado o arquivo **DockerFile** em todos os projeto, inclusive no projetos `pricing-service` e `rating-service`. Também foi adicionado o arquivo `docker-compose-yml` e `.dockerignore`.   
...
Observe que os únicos que estão com o parametros ports definido é o `api-gateway` e `consul`, sendo assim são os únicos que poderam ser acessados externamento.
...
Com já foi observado no arquivo `docker-compose` estamos iniciando um container do Consul. O `Consul` será utilizado juntamente do `OCELOT` para facilitar a descoberta de serviços na rede. 
...
Na seçãoo de configuração das rotas das API agora só é necessário informar o `ServiceName` e o `LoadBalancerOptins` e também temos que fazer referencia ao ao Serviço de Discoberta na seção `GlobalConfiguration:ServiceDiscoveryProvider` 
...
Já a propriedade `ServiceName` da configuração de `Routes` deve representar o mesmo nome de serviço configuração no arquivo` AppSettings.json`. Essa configuração é enviado para Consul para ser registrado o serviço com esse nome.
...
Acima é descrito a configuração `ServiceSettings`` do arquivo appsettings.json iremos descrever o que é cada uma das propriedades:
* **ServiceName**: Nome do serviço que será registrado no Consul. Esse nome deve ser o mesmo configuração no arquivo `Ocelot.json`
* **ServiceHost**: Nome que será utilizado para identificar a máquina que está rodando o Serviço. Se for execução local, pode ser `Localhost` ou `IP da máquina`, quando estiver usando `docker` devemos colocar o _`nome do host do container ou um de seus alias de rede`_;

![docker-compose-hostname](./imgs/docker-compose-hostname.png)

Como pode ser visto na imagem acima o hostname do serviço está definido como `book-service-host`, porém esse não é o único nome na qual o contaner responder. Utilizando o comando abaixo é possível identificar as configurações interna do container e verificar os `alias de rede` que o container responde. Seria possível informar qualquer um desses nomes na propriedade `ServiceHost`

```powershell
docker container inspect book-service-api
```

Abaixo saída do comando listado acima;
![docker-compose-alias](./imgs/docker-compose-alias.png)


* **ServicePort**: Porta que o serviço foi configurado. Essa configuração pode ser obtida na arquivo Program.cs onde é definido a propriedade `ListenAnyIP`. Quando estiver utilizando docker e o serviços estiver externo deve ser adiciona a porta que fica antes do :, conforme pode ser visto no exemplo abaixo:

![config-port-docker](./imgs/config-port-docker.png)

A porta defina com número 1 é a porta que será acessado pelo browser ou porta externa, já a porta número 2 é a porta que está configuração no arquivo `Program.cs` ou porta interna.
...
```
A classe `ServiceSettings` é um representação em objeto das configurações feita no arquivo `appsettings.json`. 
